...
while [ "$SEM" != "$(hostname)" ]; do
  while [ "$SEM" != "available" ]; do
    sleep $R
    SEM=$(aws ssm get-parameter --name "/$PREFIX/sem" --region $REGION|jq -r .Parameter.Value)
  done
  #set parameter write lock in the semaphore
  aws ssm put-parameter --name "/$PREFIX/sem" --overwrite --region $REGION --value $(hostname)
  sleep $R
  #check if lock successful
  SEM=$(aws ssm get-parameter --name "/$PREFIX/sem" --region $REGION|jq -r .Parameter.Value)
done
#get first element as TARGET and shift it to the end of array
TARGETS=($(aws ssm get-parameter --name "/$PREFIX/targets" --region $REGION|jq -r .Parameter.Value))
...
T=$(printf "%s\n" "${TARGETS[@]}")
aws ssm put-parameter --name "/$PREFIX/targets" --overwrite --region $REGION --value "$T"
#release lock
